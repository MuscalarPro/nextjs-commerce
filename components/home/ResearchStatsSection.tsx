                                    ...
                                    <div className="flex flex-col h-full">
                                       ...
                                       <div className="mt-auto pt-10 border-t border-dotted border-neutral-200 relative">
                                          <div className="flex items-center gap-2 mb-8">
                                             ...
                                             <span className="text-xs font-bold uppercase tracking-widest text-neutral-500">Number of publications (pubmed.gov)</span>
                                          </div>
                                          
                                          <div className="relative h-[300px] w-full flex items-end justify-between gap-1 md:gap-2 pr-10">
                                             {/* Background Grid Lines (Mock) */}
                                             <div className="absolute inset-0 flex flex-col justify-between pointer-events-none text-xs text-neutral-300 font-mono">
                                                <div className="w-full border-b border-dashed border-neutral-100 flex items-end"><span>600</span></div>
                                                <div className="w-full border-b border-dashed border-neutral-100 flex items-end"><span>500</span></div>
                                                <div className="w-full border-b border-dashed border-neutral-100 flex items-end"><span>400</span></div>
                                                <div className="w-full border-b border-dashed border-neutral-100 flex items-end"><span>300</span></div>
                                             </div>

                                             {/* Bars (Mock Data) */}
                                             {[20, 35, 45, 60, 80, 100, 140, 180, 220, 280, 350, 420, 500].map((h, i) => (
                                                <div 
                                                  key={i} 
                                                  className="bg-red-300 w-full rounded-t-sm relative group"
                                                  style={{ height: `${(h / 600) * 100}%`, opacity: 0.3 + (i * 0.05) }}
                                                >
                                                </div>
                                             ))}
                                             ...
                                             <div className="absolute top-[20%] left-0 text-[8rem] md:text-[10rem] font-bold text-red-700/90 leading-none">
                                                500<span className="text-[4rem] align-top relative top-4">+</span>
                                             </div>
                                          </div>
                                          
                                          <div className="mt-4 text-xs text-neutral-400 md:text-right">
                                            Pre-clinical and clinical studies have explored the potential benefits of Urolithin A
                                          </div>
                                       </div>
                                    </div>
                                 ) : (
